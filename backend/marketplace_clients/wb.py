...
class WildberriesClient:
    ...
    async def get_questions(self, is_answered: bool = False, take: int = 50, skip: int = 0) -> list[dict[str, Any]]:
        """Load WB questions with tolerant fallbacks.

        WB has changed response shapes/filters several times. In production we do not
        want a silent zero because one query shape stopped returning items. We try the
        strict request first, then safe fallbacks and keep the raw response diagnostics
        in the caller status. Duplicates are removed by id.
        """
        attempts = [
            {'isAnswered': str(is_answered).lower(), 'take': take, 'skip': skip, 'order': 'dateDesc'},
            {'isAnswered': is_answered, 'take': take, 'skip': skip, 'order': 'dateDesc'},
            {'take': take, 'skip': skip, 'order': 'dateDesc'},
        ]
        ...
        for params in attempts:
            try:
                data = await self._get('/api/v1/questions', params)
                items = self._extract_items(data, 'questions')
                for item in items:
                    if not isinstance(item, dict):
                        continue
                    # If fallback without isAnswered returns mixed data, filter locally.
                    if 'isAnswered' in item and bool(item.get('isAnswered')) != bool(is_answered):
                        continue
                    item_id = str(item.get('id') or item.get('questionId') or item.get('questionID') or '')
                    key = item_id or str(item)
                    if key in seen:
                        continue
                    seen.add(key)
                    merged.append(item)
        ...
        if not merged and errors:
            # Return empty list but keep the connector alive. The sync status will show
            # received=0; direct diagnostics can still expose count endpoints.
            return []
        return merged
