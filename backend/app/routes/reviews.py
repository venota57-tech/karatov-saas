...
def list_reviews(
    ...
    safe_limit = min(max(int(limit or 200), 1), 500)
    safe_offset = max(int(offset or 0), 0)
    return q.order_by(desc(Review.created_at_marketplace), desc(Review.created_at)).offset(safe_offset).limit(safe_limit).offset(safe_offset).limit(safe_limit).offset(safe_offset).limit(safe_limit).all()
