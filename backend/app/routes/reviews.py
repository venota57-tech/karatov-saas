...
from ..services.publishing_service import publish_review, edit_published_review_answer, publish_reviews_bulk

router = APIRouter(prefix='/reviews', tags=['reviews'])

@router.get('', response_model=list[ReviewOut])
def list_reviews(status: str | None = None, platform: str | None = None, answer_state: str = 'all', source_status: str | None = None, product: str | None = None, category: str | None = None, sentiment: str | None = None, risk: str | None = None, response_origin: str | None = None, limit: int = 500, db: Session = Depends(get_db)):
    q = db.query(Review)
    if status:
    ...
    if sentiment:
        q = q.filter(Review.ai_sentiment == sentiment)
    if risk:
        q = q.filter(Review.ai_risk_level == risk)
    if response_origin:
        q = q.filter(Review.response_origin == response_origin)
    if answer_state == 'answered':
        # Historical/analytics layer: answered + archive must be visible as a list, not only as a counter.
        q = q.filter(Review.source_status.in_(['wb_answered', 'wb_archive', 'ozon_answered']))
    elif answer_state == 'unanswered':
        # Operational queue: only items WB returned from isAnswered=false in the latest successful block.
        ...
        q = q.filter(Review.status.in_(['ready_to_review','ready_to_publish','answer_rejected_quality_gate']))
    elif answer_state == 'auto_published':
        q = q.filter(Review.status.in_(['auto_published','published']))
    return q.order_by(desc(Review.created_at_marketplace), desc(Review.created_at)).limit(limit).all()
...
def generate_review_answer(review_id: int, db: Session = Depends(get_db)):
    ...
    result = AnswerGenerator(get_rules(db).rules).generate_for_review_until_pass({
        'platform': review.platform, 'sku': review.sku, 'product_name': review.product_name,
        'rating': review.rating, 'text': review.text, 'pros': review.pros, 'cons': review.cons,
        'client_name': review.client_name, 'variation_seed': random.randint(1, 1_000_000)
    })
...
@router.post('/{review_id}/edit-published-answer')
async def edit_published_answer(review_id: int, db: Session = Depends(get_db)):
    try:
