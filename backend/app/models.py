...
class MarketplaceOperation(Base):
    ...
    status: Mapped[str] = mapped_column(String(64), default='synced', index=True)
    marketplace_status: Mapped[str | None] = mapped_column(String(128), nullable=True, index=True)
    cx_workflow_status: Mapped[str] = mapped_column(String(64), default='new_to_review', index=True)
...
class SyncCursor(Base):
    ...
    __table_args__ = (UniqueConstraint('platform', 'block', name='uq_sync_cursor_platform_block'),)

    id: Mapped[int] = mapped_column(primary_key=True)
    platform: Mapped[str] = mapped_column(String(32), index=True)
    block: Mapped[str] = mapped_column(String(128), index=True)
...
class SyncJob(Base):
    ...
    block: Mapped[str | None] = mapped_column(String(128), nullable=True, index=True)
