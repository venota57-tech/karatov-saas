...
def _upsert_review(db: Session, data: dict[str, Any]) -> str:
    ...
    if existing.has_answer and existing.status == 'new':
        existing.status = 'answered_on_marketplace'
    if existing.operational_status != 'needs_response' and existing.status in {'new', 'ready_to_review', 'ready_to_publish'}:
        existing.status = existing.status if existing.status == 'new' else 'status_changed'
    db.commit()
    return 'updated'
    ...
    try:
        ...
        for item in items:
            ...
            if has_answer_override is not None:
                data['has_answer'] = has_answer_override
            ...
            data['publish_blocked_reason'] = None if operational_status == 'needs_response' else 'Не находится в актуальной очереди WB “Ждут ответа”; публикация из этого раздела заблокирована.'
